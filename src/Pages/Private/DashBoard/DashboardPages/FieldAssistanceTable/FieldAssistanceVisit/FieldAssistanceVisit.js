...
import { FieldAssistantVisitExportTableDataAPI, FieldAssistantVisitTableDataAPI } from "../../../../../../Redux/DashboardSlice/FieldAssistantVisitSlice";
import { Pegination } from "../../../../../../Components/Pegination/Pegination";
import { FieldAssistantVisitExportData } from "./FieldAssistantVisitExportData";
...
export default function FieldAssistanceVisit() {
  ...
  useEffect(() => {
    ...
    if (YearValue !== 0) {
      dispatch(FieldAssistantVisitTableDataAPI({ data }))
    }

  }, [To, IsPost, From, IsClear, YearValue, StateDDL.ID, MonthDDL.ID, SeasonDDL.ID, CropNameDDL.ID, DistrictDDL.ID])
  ...
  useEffect(() => {
    ...
    if (YearValue !== 0) {
      dispatch(FieldAssistantVisitExportTableDataAPI({ data }))
    }

  }, [IsPost, From, IsClear, YearValue, StateDDL.ID, SeasonDDL.ID, CropNameDDL.ID, DistrictDDL.ID])

  const { tableData, isLoading } = useSelector(state => state.FieldAssistantVisitTableData)
  const { FieldAssistantVisitExporttableData, isExportLoading } = useSelector(state => state.FieldAssistantVisitExportTableData)
                  ...
                  <div className="col-md-12 grid-margin">
                    <div className="row page-heading">
                      <div className="col-12 col-lg-8 mb-xl-0 align-self-center align-items-center">
                        <h4 className="fontStyle">{`Field Visit -> ${searchName}`}</h4>
                      </div>
                      <div className="col-12 col-lg-4 mb-xl-0">
                        {
                          isExportLoading ?
                            <button className="download-table-xls-button float-end btn btn-export btn-md pt-1 pb-1 pl-3 pr-3 mx-3"
                              ...
                              />Loading</button>
                            :
                            FieldAssistantVisitExporttableData && FieldAssistantVisitExporttableData.table && FieldAssistantVisitExporttableData.table.length > 0 &&
                            <FieldAssistantVisitExportData
                              ExcelData={FieldAssistantVisitExporttableData}
                      ...
                      >
                        <thead>
                          <tr>
                            ...
                            <th>District </th>
                            <th>Crop Name</th>
                            <th>Total Field Visit Assign Target</th>
                            {searchName === "Completed" && ApiFlag === 'Completed' ?
                              <>
                                <th>Completed Field Visit Target</th>
                                <th>Pending Field Visit Target</th>
                                <th>Officer Name</th>
                              </>
                        ...
                        <tbody>
                          {
                            tableData && tableData.table && tableData.table.length > 0 ? tableData.table.map((item, i) => (
                              <tr key={i}>
                                ...
                                <td align="center">{item.totalFieldVisit ? item.totalFieldVisit : '-'}</td>
                                {searchName === "Completed" && ApiFlag === 'Completed' ?
                                  <>
                                    <td align="center">{item.completedFieldVisitTarget ? item.completedFieldVisitTarget : 0}</td>
                                    <td>{item.pendingFieldVisitTarget ? item.pendingFieldVisitTarget : '-'}</td>
                                  ...
                                  : searchName === "Pending" && ApiFlag === 'Pending' ?
                                    <>
                                      <td align="center">{item.completedFieldVisitTarget ? item.completedFieldVisitTarget : 0}</td>
                                      <td align="center">{item.pendingFieldVisitTarget ? item.pendingFieldVisitTarget : '-'}</td>
                                      {/* <td align="center">{item.employeeName ? item.employeeName : '-'}</td> */}
                                    </>
                                    :
                                    <></>
                                }
