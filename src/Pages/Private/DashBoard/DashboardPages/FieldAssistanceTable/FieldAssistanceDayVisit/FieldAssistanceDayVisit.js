...
import FieldAssistantDayPopUp from "./FieldAssistanceDayPopUp";
import FieldAssistanceDayViewPopUp from "./FieldAssistanceDayViewPopUp";
...
import {
  CropNameDDLAPI,
  CropNameDashboardDDLAPI,
  DesignationDDLAPI,
  DistrictDashboardDDLAPI,
  DistrictNameDDLAPI,
  MonthDDLAPI,
  MonthDashboardDDLAPI,
  OfficerNameDDLAPI,
  SeasonDDLAPI,
  SeasonDashboardDDLAPI,
  StateDashboardDDLAPI,
  StateNameDDLAPI,
} from "../../../../../../Redux/DDLSlice";
...
import { FieldAssistantDayVisitExportTableDataAPI, FieldAssistantDayVisitTableDataAPI } from "../../../../../../Redux/DashboardSlice/FieldAssistantDayVisitSlice";
import { Pegination } from "../../../../../../Components/Pegination/Pegination";
import { FieldAssistantVisitExportTableDataAPI } from "../../../../../../Redux/DashboardSlice/FieldAssistantVisitSlice";
import { FieldAssistantDayVisitExportData } from "./FieldAssistantDayVisitExportData";
...
export default function FieldAssistanceDayVisit() {
  ...
  const [fieldViewPopUp, setfieldViewPopUp] = useState(false);
  ...
  const fieldViewPopUpClick = () => {
    setfieldViewPopUp(true);
  };

  const handleCloseClick = () => {
    setfieldVisitPopUp(false);
    setfieldViewPopUp(false);
  };
  ...
  useEffect(() => {
    ...
    if (YearValue !== 0) {
      dispatch(FieldAssistantDayVisitTableDataAPI({ data }));
    }
  }, [IsPost, From, IsClear, YearValue, StateDDL.ID, SeasonDDL.ID, CropNameDDL.ID, DistrictDDL.ID,]);

  const { tableData, isLoading } = useSelector(
    (state) => state.FieldAssistantDayVisitTableData
  );
  ...
  useEffect(() => {
    ...
    if (YearValue !== 0) {
      dispatch(FieldAssistantDayVisitExportTableDataAPI({ data }))
    }

  }, [IsPost, From, IsClear, YearValue, StateDDL.ID, SeasonDDL.ID, CropNameDDL.ID, DistrictDDL.ID])
  const { FieldAssistantDayVisitExportTableData, isExportLoading } = useSelector(state => state.FieldAssistantDayVisitExportTableData)
  ...
  return (
    <>
                  ...
                  <div className="col-md-12 grid-margin">
                    <div className="row page-heading">
                      <div className="col-12 col-lg-8 mb-xl-0 align-self-center align-items-center">
                        <h4 className="fontStyle">{`Field Day -> ${searchName}`}</h4>
                      </div>
                      <div className="col-12 col-lg-4 mb-xl-0">
                        {
                          isExportLoading ?
                            <button className="download-table-xls-button float-end btn btn-export btn-md pt-1 pb-1 pl-3 pr-3 mx-3"
                              ...
                              />Loading</button>
                            :
                            FieldAssistantDayVisitExportTableData && FieldAssistantDayVisitExportTableData.table && FieldAssistantDayVisitExportTableData.table.length > 0 &&
                            <FieldAssistantDayVisitExportData
                              ExcelData={FieldAssistantDayVisitExportTableData}
                              name='Field Assistant Visit'
                      ...
                      <div className="card-body">
                        <div className="filter-bg p-2">
                          <div className="row">
                            ...
                            {searchName === "Target" || searchName === "Pending" ? (
                              <></>
                            ) : (
                              <>
                                <div className="col-md-6 col-lg-3">
                      ...
                      >
                        <thead>
                          <tr>
                            ...
                            <th>Season</th>
                            <th>Crop Name</th>
                            <th>Total Field Day Assign Target</th>
                            {searchName === "Completed" ? (
                              <>
                                <th>Completed Field Day Target</th>
                                <th>Pending Field Day Target</th>
                                <th>Officer Name</th>
                              </>
                            ) : searchName === "Pending" ? (
                              <>
                          ...
                          {tableData &&
                            ...
                            tableData.table.map((item, i) => (
                              <tr key={i}>
                                ...
                                <td>
                                  {item.totalFieldDayTarget ? item.totalFieldDayTarget : "-"}
                                </td>
                                {searchName === "Completed" &&
                                  ApiFlag === "Completed" ? (
                                  <>
                                    <td> {item.completedFieldDayTarget ? item.completedFieldDayTarget : 0}</td>
                                    <td>{item.pendingFieldDayTarget ? item.pendingFieldDayTarget : "-"}</td>
      ...
      {fieldVisitPopUp ? (
        <FieldAssistantDayPopUp
          open={fieldVisitPopUp}
