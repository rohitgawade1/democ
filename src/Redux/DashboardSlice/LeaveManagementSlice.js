...
// Get Table Data
export const LeaveManagementTableDataAPI = createAsyncThunk("LeaveManagementTableData", async ({ data }) => {
    const {
        FinancialYearID,
        MonthID,
        M_StateNameID,
    ...
    return fetch(`${BaseUrl}/DB/Get_Web_DB_LeaveManagement_Select?M_EmployeeLeaveID=0&M_FinancialYearID=${FinancialYearID ? FinancialYearID : '0'}&MonthID=${MonthID ? MonthID : '0'}&M_StateNameID=${M_StateNameID ? M_StateNameID : '0'}&M_DistrictNameID=${M_DistrictNameID ? M_DistrictNameID : '0'}&M_TalukaNameID=${M_TalukaNameID ? M_TalukaNameID : '0'}&LeaveDate=${LeaveDate ? LeaveDate : ''}&M_DepartmentID=${M_DepartmentID ? M_DepartmentID : '0'}&M_EmployeeID=${M_EmployeeID ? M_EmployeeID :'0'}&M_StatusID=${M_StatusID ? M_StatusID : '0'}&M_UsersID=${UserID}&Flag=${Flag}&ShowBy=${ShowBy}&FromTop=${From}&ToTop=${To}`, requestOptions)
        .then((res) => res.json())
        .then((result) => {
...
const LeaveManagementTableDataSlice = createSlice({
    name: "LeaveManagementTableData",
    initialState: {
        isLoading: false,
        tableData: null,
        isError: false,
    ...
    extraReducers: (builder) => {
        builder.addCase(LeaveManagementTableDataAPI.pending, (state, action) => {
            state.isLoading = true;
        });
        builder.addCase(LeaveManagementTableDataAPI.fulfilled, (state, action) => {
            state.isLoading = false;
            state.tableData = action.payload;
        });
        builder.addCase(LeaveManagementTableDataAPI.rejected, (state, action) => {
...
export const LeaveManagementTableDataReducer = LeaveManagementTableDataSlice.reducer
