...
// Get Table Data
export const MaterialStockCountDataAPI = createAsyncThunk("MaterialStockCountData", async ({ data,ShowBy }) => {
    const {
    ...
    return fetch(`${BaseUrl}/DB/Get_Web_DB_MaterialStockAtStoresDeptMainCount_Select?M_FinancialYearID=${M_FinancialYearID}&M_MonthID=${M_MonthID ? M_MonthID : '0'}&M_UsersID=${UserID ? UserID : '0'}&M_EmployeeID=${M_EmployeeID ? M_EmployeeID:'0'}&Flag=${Flag}&ShowBy=${ShowBy}`, requestOptions)
        .then((res) => res.json())
        .then((result) => {
            if (result.code >= 200 && result.code <= 300 && result.data) {
                handleMaterialStockGetCount(result.data.table && result.data.table)
                return result.data
            } else {
                return result
            }
...
const MaterialStockCountDataSlice = createSlice({
    name: "MaterialStockCountData",
    initialState: {
        isLoading: false,
        tableData: null,
        isError: false,
    ...
    extraReducers: (builder) => {
        builder.addCase(MaterialStockCountDataAPI.pending, (state, action) => {
            state.isLoading = true;
        });
        builder.addCase(MaterialStockCountDataAPI.fulfilled, (state, action) => {
            state.isLoading = false;
            state.tableData = action.payload;
        });
        builder.addCase(MaterialStockCountDataAPI.rejected, (state, action) => {
...
export const MaterialStockCountDataReducer = MaterialStockCountDataSlice.reducer
