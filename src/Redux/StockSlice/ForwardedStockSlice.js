...
// Get Table Data
export const ForwardedStockTableDataAPI = createAsyncThunk("ForwardedStockTableData", async ({ data }) => {
    const {
    ...
    return fetch(`${BaseUrl}/master/Get_M_Product_Stock_Distribute_Select?M_Product_CategoryID=${M_Product_CategoryID ? M_Product_CategoryID : '0'}&M_Product_SubCategoryID=${M_Product_SubCategoryID ? M_Product_SubCategoryID : '0'}&M_ProductID=${M_ProductID ? M_ProductID : '0'}&M_Product_PackDetailsID=${M_Product_PackDetailsID ? M_Product_PackDetailsID : '0'}&MfgDate=&ExpDate=&BatchNo=&M_UserID=${UserID}&FromTop=${From}&ToTop=${To}&Flag=${Flag}`, requestOptions)
        .then((res) => res.json())
        .then((result) => {
...
const ForwardedStockTableDataSlice = createSlice({
    name: "ForwardedStockTableData",
    initialState: {
        isLoading: false,
        tableData: null,
        isError: false,
    ...
    extraReducers: (builder) => {
        builder.addCase(ForwardedStockTableDataAPI.pending, (state, action) => {
            state.isLoading = true;
        });
        builder.addCase(ForwardedStockTableDataAPI.fulfilled, (state, action) => {
            state.isLoading = false;
            state.tableData = action.payload;
        });
        builder.addCase(ForwardedStockTableDataAPI.rejected, (state, action) => {
...
export const ForwardedStockTableDataReducer = ForwardedStockTableDataSlice.reducer

// Forwarded stock post Api

export const ForwardedStockPostAPI = createAsyncThunk("ForwardedStockPost", async ({ data }) => {
    const {
        M_Product_PackDetailsID,
        BatchNo,
        MfgDate,
    ...
    fetch(`${BaseUrl}/Master/Post_M_Product_Stock_Distribute_Insert`, requestOptions)
        .then((res) => res.json())
        .then((result) => {
            if (result.status) {
                handlePost()
...
const ForwardedStockPostSlice = createSlice({
    name: "ForwardedStockPost",
    initialState: {
        isLoading: false,
        ForwardedStockPostData: null,
        isError: false,
    },
    extraReducers: (builder) => {
        builder.addCase(ForwardedStockPostAPI.pending, (state, action) => {
            state.isLoading = true;
        });
        builder.addCase(ForwardedStockPostAPI.fulfilled, (state, action) => {
            state.isLoading = false;
            state.RecieveStockPostData = action.payload;
        });
        builder.addCase(ForwardedStockPostAPI.rejected, (state, action) => {
            console.log("Error", action.payload);
            state.isLoading = false;
            state.isError = true;
            state.ForwardedStockPostData = null;
            // toastErrorr('Something went wrong')
        });
    },
})

export const ForwardedStockPostReducer = ForwardedStockPostSlice.reducer

// Get Table Data
export const ForwardedOrderExportTableDataAPI = createAsyncThunk("ForwardedOrderExportTableData", async ({ data }) => {
    const {
        MonthID,
        M_FinancialYearID,
        M_DistrictNameID,
